package spaceCoder.riftcraft.dimensions.Renderers;
                ...
                for (int i1 = k2 - b0; i1 <= k2 + b0; ++i1)
                {
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow())
                    {
                        ...
                        if (l1 != i2)
                        {
                            ...
                            // float f16 = this.rand.nextFloat() + f5 * 0.01F *
                            // (float)this.rand.nextGaussian();
                            // float f11 = this.rand.nextFloat() + f5 *
                            // (float)this.rand.nextGaussian() * 0.001F;
                            // d4 = (double)((float)i1 + 0.5F) -
                            // entitylivingbase.posX;
                            // double d5 = (double)((float)l + 0.5F) -
                            // entitylivingbase.posZ;
                            // float f14 = MathHelper.sqrt_double(d4 * d4 + d5 *
                            // d5) / (float)b0;
                            // float f15 = 1.0F;
                            // tessellator.setBrightness((worldclient.getLightBrightnessForSkyBlocks(i1,
                            // j2, l, 0) * 3 + 15728880) / 4);
                            // tessellator.setColorRGBA_F(f15, f15, f15, ((1.0F
                            // - f14 * f14) * 0.3F + 0.5F) * f1);
                            // tessellator.setTranslation(-d0 * 1.0D, -d1 *
                            // 1.0D, -d2 * 1.0D);
                            // tessellator.addVertexWithUV((double)((float)i1 -
                            // f6) + 0.5D, (double)l1, (double)((float)l - f7) +
                            // 0.5D, (double)(0.0F * f8 + f16),
                            // (double)((float)l1 * f8 / 4.0F + f10 * f8 +
                            // f11));
                            // tessellator.addVertexWithUV((double)((float)i1 +
                            // f6) + 0.5D, (double)l1, (double)((float)l + f7) +
                            // 0.5D, (double)(1.0F * f8 + f16),
                            // (double)((float)l1 * f8 / 4.0F + f10 * f8 +
                            // f11));
                            // tessellator.addVertexWithUV((double)((float)i1 +
